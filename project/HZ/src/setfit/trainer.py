...
class HeadLoss(nn.Module):
    def __init__(self, model, model_head):
        super(HeadLoss, self).__init__()
        self.model = model
        self.model_head = model_head


    def forward(self, sentence_feature, labels):
        print(sentence_feature)
        embedding = self.model(sentence_feature[0])['sentence_embedding']
        output = self.model_head(embedding)
        
        return self.model_head.get_loss_fn()(output, labels.view(-1))
    ...
    def train(
        ...
        if not is_differentiable_head or self._freeze:
            ...
            if self.loss_class in [
                ...
                ps = [InputExample(texts=[x], label=y) for x, y, in zip(x_train, y_train)]
                train_dataloader2 = DataLoader(ps, shuffle=True, batch_size=16)
                train_loss2 = HeadLoss(self.model.model_body, self.model.model_head)
...
#                 from IPython import embed; embed(); exit()
            ...
            self.model.model_body.fit(
                train_objectives=[(train_dataloader, train_loss), (train_dataloader2, train_loss2)],
