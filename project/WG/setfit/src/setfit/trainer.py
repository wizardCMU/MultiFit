...
class SetFitTrainer:
    ...
    def __init__(
        ...
        #Flag for freezing the sentence transformer trainer._freeze
    ...
    def train(
        ...
        if not is_differentiable_head or self._freeze:
            ...
            if self.loss_class in [
                ...
                train_examples = []
                if self.loss_class is losses.SoftmaxLoss: is_classification = True
                ...
                if self.loss_class is losses.SoftmaxLoss:
                    train_loss = self.loss_class(
                        model=self.model.model_body, 
                        sentence_embedding_dimension=self.model.model_body.get_sentence_embedding_dimension(), 
                        num_labels=len(np.unique(y_train))
                    )
                else: train_loss = self.loss_class(self.model.model_body)
                train_steps = len(train_dataloader) * num_epochs
